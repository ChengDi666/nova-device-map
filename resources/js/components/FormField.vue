...
export default {
  ...
  mounted() {
    ...
    if(this.field.value) {  //  初始化覆盖物
      setTimeout(() => this.initAddOverlays(this.field.value), 500);
    }
  },
  methods: {
    screenZoom() {  //  放大缩小地图
    ...
    initAddOverlays(data) { //  初始化添加覆盖物
      let map = this.amapManager._map, Overlays;
      ...
      } else if (data.type == undefined && data.lng && data.lat) {
    ...
    clears(e) { //  清除覆盖物
      ...
      if(this.initoverlay) {
        map.remove(this.initoverlay);
        this.initoverlay = undefined;
      }
      map.remove(this.newOverlays);
      this.newOverlays = [];
      this.lng = '';
      this.lat = '';
    },
